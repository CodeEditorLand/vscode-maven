...
import {
	ConflictResolver,
	conflictResolver,
} from "./codeAction/conflictResolver";
...
import {
	...
	getExtensionId,
	getExtensionVersion,
	loadMavenSettingsFilePath,
	loadPackageInfo,
} from "./utils/contextUtils";
import { executeInTerminal } from "./utils/mavenUtils";
import {
	dependenciesContentUri,
	effectivePomContentUri,
	openFileIfExists,
	registerCommand,
		...
		vscode.workspace.onDidChangeWorkspaceFolders(
			async (e: vscode.WorkspaceFoldersChangeEvent) => {
				for (const removedWorkspaceFolder of e.removed) {
					await mavenExplorerProvider.removeWorkspaceFolder(
	...
	registerCommand(context, "maven.java.projectConfiguration.update", () => {
		...
		if (isJavaExtEnabled()) {
			...
			vscode.commands.executeCommand(
				...
				MavenProjectManager.projects.map((n) => Uri.file(n.pomPath))
			);
		}
	});
...
function registerConfigChangeListener(context: vscode.ExtensionContext): void {
	const configChangeListener: vscode.Disposable =
		vscode.workspace.onDidChangeConfiguration(
			async (e: vscode.ConfigurationChangeEvent) => {
				...
				if (
					...
					(e.affectsConfiguration("java.home") &&
						Settings.Terminal.useJavaHome())
				) {
...
function registerPomFileAuthoringHelpers(
	context: vscode.ExtensionContext
): void {
	...
	// hover
	context.subscriptions.push(
		vscode.languages.registerHoverProvider(pomSelector, new HoverProvider())
	);
...
async function mavenHistoryHandler(
	item: MavenProject | undefined
): Promise<void> {
	if (item) {
		await Utils.executeHistoricalGoals([item.pomPath]);
	} else {
		await Utils.executeHistoricalGoals(
			MavenProjectManager.projects.map((node) => node.pomPath)
		);
	}
}
...
async function openPomHandler(
	node: MavenProject | { uri: string }
): Promise<void> {
