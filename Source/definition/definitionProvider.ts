		...
		switch (tagNode?.tagName) {
			...
			case XmlTagName.Version: {
				...
				if (
					...
					return getDependencyDefinitionLink(
						parentNode,
						document,
						position,
					);
					...
					// parent -> artifact
					return getParentDefinitionLink(
						parentNode,
						document,
						position,
					);
...
function getParentDefinitionLinkFromRelativePath(
	parentNode: Element,
	document: vscode.TextDocument,
	position: vscode.Position,
) {
	...
	if (mavenProject) {
		...
		const originSelectionRange: vscode.Range = new vscode.Range(
			parentNode && parentNode.startIndex !== null
				? document.positionAt(parentNode.startIndex)
				: position,
			parentNode && parentNode.endIndex !== null
...
function getDependencyDefinitionLink(
	dependencyOrPluginNode: Element,
	document: vscode.TextDocument,
	position: vscode.Position,
) {
	const selectionRange: vscode.Range = new vscode.Range(
		dependencyOrPluginNode.startIndex !== null
			? document.positionAt(dependencyOrPluginNode.startIndex)
			: position,
		dependencyOrPluginNode.endIndex !== null
			? document.positionAt(dependencyOrPluginNode.endIndex)
			: position,
	);
	...
	if (groupIdHint && artifactIdHint) {
		...
		if (version !== undefined) {
			...
			if (existsSync(pomPath)) {
				const definitionLink: vscode.LocationLink = {
					targetRange: new vscode.Range(0, 0, 0, 0),
					targetUri: vscode.Uri.file(pomPath).with({
						scheme: "vscode-maven",
				...
				for (const p of pomPaths) {
					if (existsSync(p)) {
						links.push({
							targetRange: new vscode.Range(0, 0, 0, 0),
							targetUri: vscode.Uri.file(p).with({
								scheme: "vscode-maven",
								authority: "local-repository",
							}),
							originSelectionRange: selectionRange,
...
function getModuleDefinitionLink(
	moduleNode: Element,
	document: vscode.TextDocument,
	position: vscode.Position,
) {
	...
	const selectionRange: vscode.Range = new vscode.Range(
		moduleNode && moduleNode.startIndex !== null
			...
			: position,
		moduleNode && moduleNode.endIndex !== null
 ...
 */
function getParentDefinitionLink(
	parentNode: Element,
	document: vscode.TextDocument,
	position: vscode.Position,
) {
	...
	if (relativePathNode && !relativePathNode.firstChild) {
		...
		return (
			getParentDefinitionLinkFromRelativePath(
				parentNode,
				document,
				position,
