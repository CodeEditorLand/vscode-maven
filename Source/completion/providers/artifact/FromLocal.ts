...
import {
	COMMAND_COMPLETION_ITEM_SELECTED,
	INFO_COMPLETION_ITEM_SELECTED,
} from "../../constants";
...
export class FromLocal implements IArtifactCompletionProvider {
	public async getGroupIdCandidates(
		groupIdHint: string,
		...
		const commandOnSelection: vscode.Command = {
			...
			arguments: [
				{
					infoName: INFO_COMPLETION_ITEM_SELECTED,
					completeFor: "groupId",
					source: "maven-local",
				},
			],
	...
	public async getArtifactIdCandidates(
		groupId: string,
	): Promise<vscode.CompletionItem[]> {
		if (!groupId) {
			return [];
	...
	public async getVersionCandidates(
		groupId: string,
		artifactId: string,
	): Promise<vscode.CompletionItem[]> {
		...
		const validVersions: string[] = await this.searchForVersions(
			groupId,
			artifactId,
		);
	...
	private async searchForVersions(
		groupId: string,
		artifactId: string,
	): Promise<string[]> {
		const cwd: string = path.join(
