	...
	async provide(
		...
		switch (tagNode.tagName) {
			case XmlTagName.GroupId: {
				...
				const centralItems: vscode.CompletionItem[] =
					await this.centralProvider.getGroupIdCandidates(
						groupIdHint,
						artifactIdHint
					);
			...
			case XmlTagName.ArtifactId: {
				...
				const centralItems: vscode.CompletionItem[] =
					await this.centralProvider.getArtifactIdCandidates(
						groupIdHint,
						artifactIdHint
					);
				const indexItems: vscode.CompletionItem[] =
				...
				if (
					groupIdTextNode &&
					groupIdTextNode.startIndex !== null &&
					groupIdTextNode.endIndex !== null
				) {
					for (const item of mergedItems) {
						const matchedGroupId: string | undefined = _.get(
							item,
							"data.groupId"
						);
						if (matchedGroupId) {
			...
			case XmlTagName.Version: {
				...
				const groupIdHint: string = getTextFromNode(
				...
				const centralItems: vscode.CompletionItem[] =
					await this.centralProvider.getVersionCandidates(
						groupIdHint,
						artifactIdHint
					);
				const indexItems: vscode.CompletionItem[] =
...
function dedupItemsWithGroupId(
