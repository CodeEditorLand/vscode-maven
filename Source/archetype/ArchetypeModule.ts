...
import {
	...
	selectArchetypeStep,
	selectParentPomStep,
	specifyArchetypeVersionStep,
	specifyArtifactIdStep,
	specifyGroupIdStep,
	specifyTargetFolderStep,
} from "./createProject";
import {
...
const REMOTE_ARCHETYPE_CATALOG_URL =
	"https://repo.maven.apache.org/maven2/archetype-catalog.xml";

export class ArchetypeModule {
	public static async createMavenModule(
		entry: Uri | IProjectCreationMetadata | undefined,
	): Promise<void> {
	...
	public static async createMavenProject(
		entry: Uri | IProjectCreationMetadata | undefined,
	): Promise<void> {
		const metadata: IProjectCreationMetadata = {
	...
	private static async scaffoldMavenProject(
	...
	public static async updateArchetypeCatalog(): Promise<void> {
		const xml: string = await Utils.downloadFile(
	...
	public static async listArchetypeFromXml(
		xmlString: string,
	): Promise<Archetype[]> {
		try {
...
async function createBasicMavenProject(
	metadata: IProjectCreationMetadata,
): Promise<void> {
...
async function updateParentPom(
		...
		if (node && getTextFromNode(node) === "pom") {
			...
			workspaceEdit.replace(
				...
				new vscode.Range(
					...
					pomDocument.positionAt(
						packagingNodes[0].endIndex
		...
		if (modules.length) {
			...
			workspaceEdit.replace(
				...
				new vscode.Range(
					...
					pomDocument.positionAt(
						moduleNodes[0].endIndex
							? moduleNodes[0].endIndex + 1
							: 0,
					),
				),
				`<modules>\n` +
