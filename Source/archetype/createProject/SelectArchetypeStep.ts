...
export class SelectArchetypeStep implements IProjectCreationStep {
					...
					pickBox.onDidAccept(async () => {
						if (pickBox.selectedItems[0].archetype === undefined) {
							if (
								...
								pickBox.selectedItems[0].label === LABEL_MORE
							) {
								// More archetypes...
								pickBox.items =
	...
	private async getArchetypePickItems(
		all?: boolean,
	): Promise<IArchetypePickItem[]> {
		...
		if (!fixedList) {
			...
			return fixedList
				.map((fullname: string) =>
					allItems.find(
						(item: Archetype) =>
							fullname === `${item.groupId}:${item.artifactId}`,
					),
				)
	...
	private async getLocalArchetypeItems(): Promise<Archetype[]> {
		const localCatalogPath: string = path.join(
			getMavenLocalRepository(),
	...
	private async getCachedRemoteArchetypeItems(): Promise<Archetype[]> {
		const contentPath: string = getPathToExtensionRoot(
			"resources",
			"archetypes.json",
		);
