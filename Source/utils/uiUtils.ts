...
const TROUBLESHOOTING_LINK =
	"https://github.com/Microsoft/vscode-maven/blob/master/Troubleshooting.md";

// file chooser dialog
export async function openDialogForFolder(
...
export async function openDialogForFile(
	customOptions?: OpenDialogOptions,
): Promise<Uri | undefined> {
	const options: OpenDialogOptions = {
		canSelectFiles: true,
...
// Troubleshooting
export async function showTroubleshootingDialog(
	errorMessage: string,
): Promise<void> {
	...
	if (choiceForDetails === OPTION_LEARN_MORE) {
		...
		vscode.commands.executeCommand(
			"vscode.open",
			vscode.Uri.parse(TROUBLESHOOTING_LINK),
		);
...
export async function selectProjectIfNecessary(): Promise<
	MavenProject | undefined
> {
	if (
		MavenProjectManager.projects === undefined ||
		MavenProjectManager.projects.length === 0
	) {
		return undefined;
	}
...
export function registerCommand(
	...
	withOperationIdAhead?: boolean,
): void {
	const callbackWithTroubleshooting: (...args: unknown[]) => unknown =
		instrumentOperation(
			commandName,
			async (_operationId: string, ...args: unknown[]) => {
				try {
					return withOperationIdAhead
						? await func(_operationId, ...args)
						: await func(...args);
				} catch (error) {
...
export function registerCommandRequiringTrust(
	context: vscode.ExtensionContext,
	commandName: string,
	func: (...args: unknown[]) => unknown,
	withOperationIdAhead?: boolean,
