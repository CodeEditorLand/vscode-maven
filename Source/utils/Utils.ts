...
export class Utils {
	public static async parseXmlFile(
		xmlFilePath: string,
	...
	public static async parseXmlContent(
		xmlString: string,
		options?: xml2js.OptionsV2,
	): Promise<unknown> {
				...
				const options: object = Object.assign(
					{
						headers: Object.assign({}, customHeaders, {
							"User-Agent": `vscode/${getExtensionVersion()}`,
						}),
					},
					urlObj,
				);
	...
	public static async showEffectivePom(
	...
	public static async getEffectivePom(
	...
	public static async getPluginDescription(
		pluginId: string,
		pomPath: string,
	): Promise<string> {
		const task = async (p: Progress<{ message?: string }>) => {
	...
	public static async executeCustomGoal(
		...
		if (typeof pomOrProject === "string") {
			...
			typeof pomOrProject === "object" &&
			pomOrProject instanceof MavenProject
		) {
	...
	public static async executeHistoricalGoals(
		projectPomPaths: string[],
	): Promise<void> {
		...
		if (selected) {
			await executeInTerminal({
				...
				pomfile: selected.pomPath,
			});
		}
	}

	public static async executeMavenCommand(
		...
		await commands.executeCommand(
			`maven.goal.${selectedCommand}`,
			selectedProject,
		);
	}

	public static parseProfilesOutput(
