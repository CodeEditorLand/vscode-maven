...
import {
	...
	getPathToTempFolder,
	getPathToWorkspaceStorage,
} from "./contextUtils";
import { MavenNotFoundError } from "./errorUtils";
import { updateLRUCommands } from "./historyUtils";

// calculate dependency graph
const GOAL_DEPENDENCY_GRAPH =
	"com.github.ferstl:depgraph-maven-plugin:4.0.2:graph";
const OPTIONS_DEPENDENCY_GRAPH =
	"-DgraphFormat=text -DshowDuplicates -DshowConflicts -DshowVersions -DshowGroupIds";
...
export async function pluginDescription(
	...
	await executeInBackground(
		`-B -Dapple.awt.UIElement=true -Dplugin=${pluginId} -Doutput="${outputPath}" help:describe`,
		pomPath
	);
	return await readFileIfExists(outputPath);
}

export async function rawProfileList(
	pomPath: string
): Promise<string | undefined> {
	const outputPath: string = getTempFolder(pomPath);
...
async function executeInBackground(
	...
	const mvnSettingsArg: string | undefined = mvnSettingsFile
		? `-s "${await mavenTerminal.formattedPathForTerminal(
				mvnSettingsFile
		  )}"`
		: undefined;
...
export async function executeInTerminal(options: {
	...
	const mvnString: string = wrappedWithQuotes(
		await mavenTerminal.formattedPathForTerminal(mvn)
	);
	const mvnSettingsFile: string | undefined = Settings.getSettingsFilePath();
	...
	const fullCommand: string = [
		...
		command.trim(),
		pomfile &&
			`-f "${await mavenTerminal.formattedPathForTerminal(pomfile)}"`,
		profileOptions,
	...
	const name: string =
		terminalName ||
		(workspaceFolder ? `Maven-${workspaceFolder.name}` : "Maven");
	const terminal: vscode.Terminal = await mavenTerminal.runInTerminal(
		fullCommand,
		{ name, cwd, env, workspaceFolder }
	);
	if (pomfile) {
...
export async function getMaven(pomPath?: string): Promise<string | undefined> {
	const mvnPathFromSettings: string | undefined =
		Settings.Executable.path(pomPath);
	if (mvnPathFromSettings) {
		// expand tilde to deal with ~/path-to-mvn
		return expandHome(mvnPathFromSettings);
	}

	const preferMavenWrapper: boolean =
		Settings.Executable.preferMavenWrapper(pomPath);
	if (preferMavenWrapper && pomPath && vscode.workspace.isTrusted) {
		const localMvnwPath: string | undefined = await getLocalMavenWrapper(
...
async function getLocalMavenWrapper(
...
export async function promptToSettingMavenExecutable(): Promise<void> {
	...
	const choice: string | undefined =
		await vscode.window.showInformationMessage(
			MESSAGE,
			BUTTON_GOTO_SETTINGS,
			BUTTON_BROWSE_FOR_MAVEN
		);
	switch (choice) {
		case BUTTON_GOTO_SETTINGS: {
			await vscode.commands.executeCommand(
				"workbench.action.openSettings",
				SETTING_MAVEN_EXECUTABLE_PATH
			);
			break;
		}
...
async function browseForMavenBinary(): Promise<string | undefined> {
	...
	const selectedUris: vscode.Uri[] | undefined =
	...
	const mvnPath: string | undefined =
		selectedUris.length > 0 && selectedUris[0] !== undefined
			? selectedUris[0].fsPath
			: undefined;
	if (!mvnPath || !mvnPath.endsWith(mvnFilename)) {
		await vscode.window.showErrorMessage(
			`Maven executable file must match with name: ${mvnFilename}`
		);
		return undefined;
	}
