...
export async function applyWorkspaceEdit(
	...
	if (workspaceEdit !== undefined) {
		...
		try {
			const currentEditor: TextEditor | undefined =
			...
			if (
				...
				currentEditor.document.uri.fsPath !==
					workspaceEdit.entries()[0][0].fsPath
			) {
				return;
			}
			...
			let startPosition: Position = new Position(
...
async function executeRangeFormat(
	editor: TextEditor,
	startPosition: Position,
	lineOffset: number,
): Promise<void> {
	const endPosition: Position = editor.document.positionAt(
...
export function constructDependenciesNode(options: {
	...
	const dependencyNode = constructDependencyNode({
		gid,
		aid,
		version,
		dtype,
