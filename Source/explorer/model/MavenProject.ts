...
export class MavenProject implements ITreeItem {
	...
	public get groupId(): string {
		return (
			this._pom?.project?.groupId?.[0] ??
			this._pom?.project?.parent?.[0]?.groupId?.[0] ??
			this.parent?.groupId
		);
	}
	...
	public get version(): string {
		return (
			this._pom?.project?.version?.[0] ??
			this._pom?.project?.parent?.[0]?.version?.[0] ??
			this.parent?.version
		);
	}
	...
	public get moduleNames(): string[] {
		const moduleNames: string[] | undefined = _.get(
			this._pom,
	...
	public async getEffectivePom(options?: {
		cacheOnly?: boolean;
	...
	private updateProperties(): void {
		if (this?._pom?.project?.properties?.[0] !== undefined) {
			for (const [key, value] of Object.entries<any>(
				this._pom.project.properties[0],
			)) {
	...
	public getProperty(key: string): string | undefined {
		...
		let cur: MavenProject | undefined =
			(this.parentPomPath
