...
class PluginInfoProvider {
	...
	public async getPluginGoals(
		...
		const desc: PluginDescription | undefined = await parseMetadataFromJar(
			groupId,
			artifactId,
			version,
		);
		if (desc) {
			...
			const { prefix, goals } = await this.parseFromPluginDescription(
				pomPath,
				groupId,
				artifactId,
				version,
	...
	private async udpatePluginInfoCache(
		gid: string,
	...
	private async parseFromPluginDescription(
...
async function parseMetadataFromJar(
	groupId: string,
	artifactId: string,
	version: string,
): Promise<PluginDescription | undefined> {
	const jarFilePath = path.join(
