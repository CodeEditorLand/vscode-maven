...
export function registerArtifactSearcher(
	...
	if (javaExt) {
		...
		registerCommand(
			context,
			COMMAND_SEARCH_ARTIFACT,
			async (param: any) => await resolver.pickAndAddDependency(param)
		);
		...
		context.subscriptions.push(
			vscode.languages.registerCodeActionsProvider("java", {
				...
				> {
					return resolver.getArtifactsCodeActions(
						document,
						codeActionContext,
						range
					);
...
class TypeResolver {
	...
	public getArtifactsHover(
		...
		const diagnostics: vscode.Diagnostic[] = vscode.languages
		...
		if (diagnostics.length > 0) {
			...
			const message = `\uD83D\uDC49 [${commandName}](command:${COMMAND_SEARCH_ARTIFACT}?${encodeURIComponent(
...
async function getArtifactsPickItems(
	className: string
...
async function applyEdits(uri: vscode.Uri, edits: any): Promise<void> {
	...
	if (Object.keys(edits[2].changes).length > 0) {
		...
		const startLine: number =
			edits[2].changes[Object.keys(edits[2].changes)[0]][0].range.start
				.line + LINE_OFFSET; // skip blank line
		const lineNumber: number =
			edits[2].changes[
				Object.keys(edits[2].changes)[0]
		...
		vscode.window.showInformationMessage(
			"Sorry, the pom.xml file is inexistent or invalid."
		);
	}
}

async function getWorkSpaceEdits(
	pickItem: vscode.QuickPickItem,
	param: any
...
function diagnosticIndicatesUnresolvedType(
	diagnostic: vscode.Diagnostic,
	document: vscode.TextDocument
): boolean {
	return (
		UNDEFINED_TYPE === diagnostic.code ||
		(UNDEFINED_NAME === diagnostic.code &&
			startsWithCapitalLetter(document.getText(diagnostic.range)))
	);
}
