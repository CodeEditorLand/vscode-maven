...
export class ConflictResolver implements vscode.CodeActionProvider {
	...
	public provideCodeActions(
		...
		return context.diagnostics
			.filter(
				(diagnostic) => diagnostic.code === MAVEN_DEPENDENCY_CONFLICT,
			)
			.map((diagnostic) =>
				this.createCommandCodeAction(diagnostic, document),
			);
	}
	...
	private createCommandCodeAction(
		...
		const actionSetVersion = new vscode.CodeAction(
			...
			arguments: [
				{
					...
					groupId: gid,
					artifactId: aid,
					fullDependencyText: MavenProjectManager.get(
						document.uri.fsPath,
					)?.fullText,
				},
			],
