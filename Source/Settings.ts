...
export class Settings {
	...
	public static enableConflictDiagnostics(): boolean {
		return !!_getMavenSection<boolean>(
			"dependency.enableConflictDiagnostics",
		);
	}
	...
	public static changeToHierarchicalView(): void {
		workspace
			.getConfiguration()
			.update("maven.view", "hierarchical", false);
	}
	...
	public static setMavenExecutablePath(mvnPath: string): void {
		workspace
			.getConfiguration()
			.update("maven.executable.path", mvnPath, true);
	}
	...
	public static Terminal = class {
		...
		): FavoriteCommand[] | undefined {
			type Favorite = { alias: string; command: string; debug?: boolean };

			return _getMavenSection<Favorite[]>(
				"terminal.favorites",
				vscode.Uri.file(project.pomPath),
			)?.map(
				(favorite) =>
					new FavoriteCommand(
						project,
	...
	public static Executable = class {
		...
		public static options(
			...
			const options: string | string[] | undefined = _getMavenSection(
				"executable.options",
				resourceOrFilepath,
			);
	...
	public static Pomfile = class {
		public static autoUpdateEffectivePOM(): boolean {
			return !!_getMavenSection<boolean>(
				"pomfile.autoUpdateEffectivePOM",
			);
		}
	...
	public static getEnvironment(resourceOrFilepath?: Uri | string): {
		[key: string]: string;
	} {
		...
		const environmentSettings: EnvironmentSetting[] | undefined =
			Settings.Terminal.customEnv(resourceOrFilepath);
