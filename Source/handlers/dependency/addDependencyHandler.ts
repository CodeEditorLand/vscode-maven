...
export async function addDependencyHandler(options?: any): Promise<void> {
	...
	if (options && options.groupId && options.artifactId) {
		...
		const keywordString: string | undefined =
		...
		const selectedDoc: IArtifactMetadata | undefined = await vscode.window
			.showQuickPick<vscode.QuickPickItem & { value: IArtifactMetadata }>(
