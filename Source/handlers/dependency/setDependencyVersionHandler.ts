...
export async function setDependencyVersionHandler(
	...
	let selectedVersion: string | undefined = await vscode.window
		.showQuickPick(
			versions.map((version) => ({
				value: version,
				label:
					version !== OPTION_SEARCH_MAVEN_CENTRAL
						? `$(package) ${version}`
						: version,
				description:
					version === effectiveVersion ? "effective" : undefined,
			})),
			{
	...
	if (selectedVersion === OPTION_SEARCH_MAVEN_CENTRAL) {
		const selectedVersionFromMavenCentral: string | undefined =
			await vscode.window
				.showQuickPick<vscode.QuickPickItem & { value: string }>(
					getVersions(gid, aid).then((artifacts) =>
						artifacts.map((artifact) => ({
							value: artifact.v,
							label: `$(package) ${artifact.v}`,
							description:
								artifact.v === effectiveVersion
									? "effective"
									: undefined,
						})),
					),
...
async function setDependencyVersion(
	pomPath: string,
	gid: string,
	aid: string,
	version: string,
	...
	const pomDocument = await vscode.window.showTextDocument(
	...
	const dependencyManagementNode: Element | undefined =
		projectNode.children.find(
			(elem) =>
				isTag(elem) && elem.tagName === XmlTagName.DependencyManagement,
		) as Element | undefined;
	// find ${gid:aid} dependency node in <dependencies> to delete
	const deleteNode = getDependencyNodeFromDependenciesNode(
		dependenciesNode,
		gid,
		aid,
		project,
...
async function insertDependencyManagement(
	...
	if (
		...
		dependencyNodeInManagement.endIndex !== null
	) {
