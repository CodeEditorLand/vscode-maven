...
export async function getDependencyNode(
	pomPath: string,
	gid: string,
	aid: string
) {
...
export function getDependencyNodeFromDependenciesNode(
	dependenciesNode: Element | undefined,
	gid: string,
	aid: string,
	project?: MavenProject
) {
	const dependencyNode = dependenciesNode?.children?.find(
		(node) =>
			isTag(node) &&
			...
			node.children?.find(
				(id) =>
					...
					(project
						? project.fillProperties(id.firstChild.data)
